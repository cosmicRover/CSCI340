/* 
Joy Paul
project 4
*/


#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#define NAME_CAP 128
#define OP_LIMIT 8

typedef struct MemoryModel
{
    size_t lo, hi;
    char *name;
    struct MemoryModel *prev, *next;
} MemoryModel;

size_t mem_size = 0;
MemoryModel *mem;

/* Allocate memory */
MemoryModel *allocateMemoryBlock(size_t lo, size_t hi, const char *name, MemoryModel *prev, MemoryModel *next)
{
    MemoryModel *ret = malloc(sizeof(MemoryModel));
    if (ret == NULL)
    {
        printf("Failed to allocate phisical memory.\n");
        exit(-1);
    }
    ret->lo = lo, ret->hi = hi;
    // allocate memory and copy the string
    if (strlen(name) != 0)
    {
        ret->name = malloc(sizeof(char) * (strlen(name) + 1));
        strcpy(ret->name, name);
    }
    else
    { // unused block
        ret->name = NULL;
    }
    // handle the prev and next to preserve a doubly-linked list
    ret->prev = prev, ret->next = next;
    if (prev)
    {
        prev->next = ret;
    }
    if (next)
    {
        next->prev = ret;
    }
    return ret;
}

/* function to request memory */
int requestMemory(const char *name, size_t size, char strategy)
{
    MemoryModel *hole = NULL;
    // select the hole
    switch (strategy)
    {

    case 'B':
    {
        MemoryModel *cursor = mem;
        size_t min_size = -1; // get the max number in size_t
        while (cursor)
        {
            size_t hole_size = (cursor->hi - cursor->lo + 1);
            if (cursor->name == NULL && size <= hole_size && hole_size < min_size)
            {
                min_size = hole_size;
                hole = cursor;
            }
            cursor = cursor->next;
        }
        break;
    }

    default:
    {
        printf("Only best fit B. Unknown : %c\n", strategy);
        return -1;
    }
    }
    if (!hole || hole->name != NULL)
    {
        printf("No available memory to allocate.\n");
        return -2;
    }
    hole->name = malloc(sizeof(char) * (strlen(name) + 1));
    strcpy(hole->name, name);
    if (hole->hi - hole->lo + 1 == size)
    { // the hole size is exactly equal to the requested size
        return 0;
    }
    hole->next = allocateMemoryBlock(hole->lo + size, hole->hi, "", hole, hole->next);
    hole->hi = hole->lo + size - 1;
    return 0;
}

// release all blocks with the given name, and do the merges if possible
int releaseMemory(const char *name)
{
    MemoryModel *cursor = mem;
    int flag = 1;
    while (cursor)
    {
        if (cursor->name && strcmp(cursor->name, name) == 0)
        {
            free(cursor->name);
            cursor->name = NULL; // mark it unused
            flag = 0;
        }
        // merge with the prev block if possible
        if (cursor->name == NULL && cursor->prev && cursor->prev->name == NULL)
        {
            MemoryModel *temp = cursor->prev;
            cursor->prev = temp->prev;
            if (temp->prev)
            {
                temp->prev->next = cursor;
            }
            cursor->lo = temp->lo;
            free(temp);
        }
        // update the first block in memory if necessary
        if (cursor->prev == NULL)
        {
            mem = cursor;
        }
        cursor = cursor->next;
    }
    if (flag)
    {
        printf("No memory gets released!\n");
    }
    return flag;
}

void compactMemory()
{
    MemoryModel *cursor = mem;
    while (cursor)
    {
        // unused --> used, swap these two blocks
        if (cursor->name && cursor->prev && !cursor->prev->name)
        {
            MemoryModel *prev = cursor->prev;
            prev->hi = prev->lo + (cursor->hi - cursor->lo);
            cursor->lo = prev->hi + 1;
            prev->name = cursor->name;
            cursor->name = NULL;
        }
        // unused --> unused, merge thees two blocks
        if (!cursor->name && cursor->prev && !cursor->prev->name)
        {
            MemoryModel *prev = cursor->prev;
            cursor->lo = prev->lo;
            cursor->prev = prev->prev;
            if (cursor->prev)
            {
                cursor->prev->next = cursor;
            }
            free(prev);
        }
        cursor = cursor->next;
    }
}

/* wrappers to help call functions */
void requestWrapper()
{
    char name[NAME_CAP], strategy;
    size_t size;
    scanf("%s %zu %c", name, &size, &strategy); // unsafe but convenient
    printf(requestMemory(name, size, strategy) ? "FAILURE\n" : "SUCCESS\n");
}

void releaseWrapper()
{
    char name[NAME_CAP];
    scanf("%s", name); // unsafe but convenient
    printf(releaseMemory(name) ? "FAILURE\n" : "SUCCESS\n");
}

/* */
void displayUsage()
{
    printf("------------------------------------------------------------\n");
    printf("<program> <memory size (in bytes)>\n");
    printf("Operations:\n");
    printf(
        "    RL <process name>\n"
        "        Release the process's contigous block of memory\n"
        "    RQ <process name> <memory size (in bytes)> <strategy>\n"
        "        Request for a contigous block of memory (available strategies are F, W and B)\n"
        "    C -> Compact unused holes of memory into one single block\n"
        "    STAT -> Report the regions of free and allocated memory\n"
        "    QUIT -> Exits the program\n");
    printf("------------------------------------------------------------\n");
}

/* outputs the results */
void displayMemory()
{
    printf("***********************************************************\n");
    MemoryModel *cursor = mem;
    while (cursor)
    {
        printf("[%06zu - %06zu] ", cursor->lo, cursor->hi);
        if (cursor->name)
        {
            printf("Process %s\n", cursor->name);
        }
        else
        {
            printf("Unused\n");
        }
        cursor = cursor->next;
    }
    printf("***********************************************************\n");
}

/* init the program with a block of memory*/
int init(int argc, char **argv)
{
    if (argc != 2)
    {
        printf("Incorrect number of arguments..\n");
        return -1;
    }
    sscanf(argv[1], "%zu", &mem_size);
    mem = allocateMemoryBlock(0, mem_size - 1, "", NULL, NULL);
    printf("The size of memory is initialized to %zu bytes\n", mem_size);
    return 0;
}

void cleanUp()
{
    MemoryModel *temp = mem;
    while (mem)
    {
        free(mem->name);
        temp = mem;
        mem = mem->next;
        free(temp);
    }
}

/*Start getting commands and calling functions */
int main(int argc, char **argv)
{
    displayUsage();
    if (init(argc, argv) != 0)
    {
        displayUsage();
        return 0;
    }
    char op[OP_LIMIT];
    while (1)
    {
        printf("allocator: type command -> ");
        scanf("%s", op); // unsafe but convenient
        if (strcmp(op, "RQ") == 0)
        {
            requestWrapper();
        }
        else if (strcmp(op, "RL") == 0)
        {
            releaseWrapper();
        }
        else if (strcmp(op, "C") == 0)
        {
            compactMemory();
        }
        else if (strcmp(op, "STAT") == 0)
        {
            displayMemory();
        }
        else if (strcmp(op, "QUIT") == 0)
        {
            cleanUp();
            break;
        }
        else
        {
            printf("Unrecognized operation.\n");
            displayUsage();
        }
        // display_memory();
    }
    return 0;
}